...
if(navigator.userAgent){
	var sc_ua = navigator.userAgent;
	if(sc_ua.match(/facebookexternalhit/) || sc_ua.match(/Google Web Preview/) || sc_ua.match(/Googlebot/)){
		s.t=function(){return '';}
	}
}
s.charSet="UTF-8";
s.cookieDomainPeriods=location.host.match(/\.(ac|go|co|ne|or|ad|gr|ed)\.jp$/)?"3":"2"
s.visitorNamespace="gurunavi"
s.trackingServer="gurunavi.d1.sc.omtrdc.net"
s.visitorMigrationKey="4D9496EF"
s.visitorMigrationServer="gurunavi.112.2o7.net"
...
s.linkInternalFilters="javascript:,ssl.gnavi.co.jp,gnavi.co.jp,r.gnavi.co.jp,enjoytokyo.jp,ekikara.jp";
...
/* 2008-11-19 */
if(!(location.host+location.pathname).match(/shop\.gnavi\.co\.jp\/Mall\/List/)){
...
/* click quality */
if(s.getCk("sc_quality")=="land"){
    s.setCk("sc_quality","pass");
}
...
function s_doPlugins(s) {
  ...
  if (s.getCk("GMember")) {
    s.prop6="Registered";
    s.setCk("scReg","true",90);
  } else if(s.getCk("scReg")) {
    s.prop6="Registered";
    s.setCk("scReg","true",90);
  } else {
    s.prop6="Non-Registered";
  }
  s.eVar6="D=c6";

  /* new repeat */
  s.prop7=s.getNewRepeat(90);
  s.eVar7=s.prop7;

	/* time parting + display00 */
	function ddDate(){
		var dddate = new Date();
		var ddh = dddate.getHours();
		var ddm = dddate.getMinutes();
		var dds = dddate.getSeconds();
		if(ddh < 10){ ddh = "0" + ddh; }
		if(ddm < 10){ ddm = "0" + ddm; }
		if(dds < 10){ dds = "0" + dds; }
		var ddsw = ddh +":"+ ddm +":"+dds ;
		return ddsw;
	}
	try{
		var dd = new Date();
		...
		if(mm < 10) { mm = '0' + mm; }
		if(day < 10) { day = '0' + day; }
		s.prop8=s.getTimeParting("w","9",dd.getFullYear())+"/"+ddDate()+"/"+s.getTimeParting("d","9",dd.getFullYear()) + "/" + dd.getFullYear().toString() + mm + day;
		s.eVar8="D=c8";
	}catch(e){}

	/* Login Count 2013-08-28 */
	s.prop64 = 'unlogin';
	var scgcom = '';
	var scguser_type = '';
	var scguser_val = '';
	scgcom = s.getCk('gcom');
	scgcom = scgcom.replace('{' , '');
	scgcom = scgcom.replace('}' , '');
	scguser_type = scgcom.split(':');
	scguser_type[1] ? scguser_val = scguser_type[1] : '';
	(scguser_val.match(/[1-2]/)) ? s.prop64 = 'login' : s.prop64 = 'unlogin';
	s.eVar64 = s.prop64;
  ...
  /* campaign tracking */
  s.campaign = s.getValOnce(s.getQueryParam("sc_cid,sc_bid,sc_mid"), "sc_campaign", 0);
  s.eVar1 = s.crossVisitParticipation(s.campaign,"sc_evar1","90","5",">","event9,purchase");
    
  /* URL hier */
  s.hier1=location.host+location.pathname;
  s.hier1=s.hier1.replace(/\/index\.\w+$/,"/");
  ...
  if(window.s_postPlugins){
	...
	/* pageName + palameter 2012-11-20 */
	s.eVar24 = s.prop24 = 'D=g';

  /* campign quality */
  if(s.campaign){
    s.setCk("sc_quality","land");
  }

	/* gnewstrend cookie取得 2013-06-05 */
	var sc_prop5 = '';
	s.getCk('gnewstrend') ? sc_prop5 = s.getCk('gnewstrend') : '';
	if(sc_prop5 !== '') {
		sc_prop5 = sc_prop5.replace(/^word=/ ,'');
		s.eVar5 = s.prop5 = "gn:" + sc_prop5;
	}

  /* prop1 cross-visit */
  s.prop50="";
  var scgpv1=s.getPreviousValue(s.prop1,"scgpv1");
  s.setCk("sc_prop1",s.prop1,0); //keep compatibility
  if(s.prop1!="r"&&s.prop1!="rm"&&s.prop1!="rp"
     &&scgpv1!="r"&&scgpv1!="rm"&&scgpv1!="rp"){
    s.prop50=s.getCk("sc_prop50");
    var scprop50lst = s.prop50.split(">");
    var scprop50len = scprop50lst.length;
    var scprop50push = new Array();
    if(scprop50lst[scprop50len-1]!=s.prop1){
			if(s.prop1 === 'mobile/iphone') {
				if(scprop50lst[scprop50len-1] === s.prop2) {
				} else {
					scprop50lst.push(s.prop2);
				}
			} else {
				scprop50lst.push(s.prop1);
			}
      scprop50len = scprop50len + 1;
    }
  /* check length 5 */
    if(scprop50len > 5){
      var scstart = scprop50len - 5;
      for(var sci=scstart;sci<scstart+5;sci++){
	scprop50push.push(scprop50lst[sci]);
      }
      s.prop50 = scprop50push.join(">");
    }else{
      for(var sci=0;sci<scprop50len;sci++){
	if(scprop50lst[sci]){
	  scprop50push.push(scprop50lst[sci]);
	}
      }
  ...
  }else if (s.prop1==="r" && s.prop2==="rp/s") {
		s.prop50=s.getCk("sc_prop50");
		var scprop50lst = s.prop50.split(">");
		var scprop50len = scprop50lst.length;
		var scprop50push = new Array();
  ...
  s.prop50=s.getAndPersistValue(s.prop50,"sc_prop50",0);
  s.eVar50="D=c50"
  s.prop49=s.prop50.substring(s.prop50.lastIndexOf(">")+1);
  s.eVar49="D=c49"
  ...
  /* group events  & hi-level path */
  if(s.events.match(/event9(,|$)/)){
    if(!s.getCk(s.channel + "event9")) {
      s.events = s.apl(s.events, "event13", ",", 1);
      s.setCk(s.channel + "event9", "1", 0);
    }
  } else if(s.events.match(/event7(,|$)/)){
    if(!s.getCk(s.channel + "event7")) {
      s.events = s.apl(s.events, "event14", ",", 1);
      s.setCk(s.channel + "event7", "1", 0);
    }
  } else if(s.events.match(/purchase/) && !s.events.match(/event16/) ){
    s.events = s.apl(s.events, "event21", ",", 1);
  }
  
  if(s.events.match(/event(8(,|$)|13|14|15|16|21|22|23|24|26|27|28)/)){
    s.events = s.apl(s.events, "event25", ",", 1);
    // success event name
    if (s.events.match(/13/)) {
      s.eVar12 = s.channel + ":" + "shoptop";
    } else if (s.events.match(/8/)) {
      s.eVar12 = s.channel + ":" + "map";
    } else if (s.events.match(/14/)) {
      s.eVar12 = s.channel + ":" + "mapprint";
    } else if (s.events.match(/15/)) {
      s.eVar12 = s.channel + ":" + "mobile";
    } else if (s.events.match(/16/)) {
      s.eVar12 = s.channel + ":" + "reservation";
    } else if (s.events.match(/21/)) {
      s.eVar12 = s.channel + ":" + "purchase";
    } else if (s.events.match(/22/)) {
      s.eVar12 = s.channel + ":" + "signup";
    } else if (s.events.match(/23/)) {
      s.eVar12 = s.channel + ":" + "kuchikomi";
    } else if (s.events.match(/24/)) {
      s.eVar12 = s.channel + ":" + "myrestaurant";
    } else if (s.events.match(/26/)) {
      s.eVar12 = s.channel + ":" + "login";
    } else if (s.events.match(/27/)) {
      s.eVar12 = s.channel + ":" + "submit";
    } else if (s.events.match(/28/)) {
      s.eVar12 = s.channel + ":" + "update";
    }
...
/* ページスクロール取得 2012-02-20 */
	if(typeof(s.getPercentPageViewed) === 'function'){
		var ppvA = s.getPercentPageViewed(s.pageName);
		var sc_prePage = ppvA[0];
		var sc_preTotal = ppvA[1];
		var sc_preInit = ppvA[2];
		if(s.c_r('s_pxh')){
			var sc_perPx = s.c_r('s_pxh');
		} else {
			var sc_perPx = '';
		}
		var sc_per100 = [];
		sc_per100 = sc_perPx.split('/');
		if(ppvA[1] == '100' && (sc_per100[0] != sc_per100[1])){
			sc_perPx = sc_per100[1] + '/' + sc_per100[1];
		}
		var gsv = sc_preInit + '%:' + sc_preTotal + '%:' + sc_perPx + ':' + sc_prePage;
		if(gsv.match(/undefined/) || gsv.match(/^%:%:/)){
			gsv = gsv.replace(/undefined/g, '');
			if(gsv.match(/^%:%::?/) || s.prop3 === '^%:%:'){
				s.prop3 = '';
			}
		}	else {
			s.prop3 = gsv;
		}
        if(s.prop3.length > 100){
			s.prop3 = s.prop3.substring(0, 100);
		}
		if(sc_prePage){
			s.eVar17 = sc_prePage;
		}
	...
	/* 404notfound 2012-11-20 */
	if(s.pageType && s.pageType.match(/errorPage/)) {
		s.pageName = s.prop1 + ':404';
	}

	/* no-result 2012-11-20 */
	if(document.forms['search_result'] && document.forms['search_result'].result) {
		if(document.forms['search_result'].result.value.match(/no_result/)){
			s.eVar61 = s.prop61 = document.forms['search_result'].result.value;
		}
	}

	/* T&T Plugins 2012-06-27 */
	s.tnt=s.trackTNT();
}

/* prop51用ページ名自動生成・UserAgent取得 2011-04-18 */
	var scpageName="";
	...
	if(sc_ua){
		...
		if(s.prop52.match(/iPhone/)){
			s.prop53 = 'iPhone';
			s.eVar53 = s.prop53;
		} else if(s.prop52.match(/iPad/)){
			s.prop53 = 'iPad';
			s.eVar53 = s.prop53;
		} else if(s.prop52.match(/Android/)){
			if(!s.prop52.match(/Mobile/i)){
				s.prop53 = 'Android Tablet';
				s.eVar53 = s.prop53;
			} else {
				s.prop53 = 'Android';
				s.eVar53 = s.prop53;
			}
		} else if(s.prop52.match(/Windows Phone/)){
			s.prop53 = 'Windows Phone';
			s.eVar53 = s.prop53;
		} else {
			s.prop53 = 'PC';
			s.eVar53 = s.prop53;
		}
		if(s.prop52.length > 100){
			s.prop52 = s.prop52.substring(0, 100);
		}
	}

s.doPlugins=s_doPlugins

/* オンクリック時に任意の値をcookieに保持 2013-08-13 */
function sc_countCk(a,b,c,d,p) {
	var sc_arg = [];
	a ? sc_arg.push(a) : '';
	b ? sc_arg.push(b) : '';
	c ? sc_arg.push(c) : '';
	d ? sc_arg.push(d) : '';
	p ? sc_arg.push(s.pageName) : '';
	sc_arg.join(',');
	(s.setCk && sc_arg !== '') ? (sc_arg ? s.setCk('sc_onck', sc_arg) : '') : '';
}

/* onunload呼び出し用 */
function sc_countRead(k) {
	if(s.getCk(k)) {
		s.linkTrackVars = "prop5,eVar5";
		s.getCk(k) ? s.prop5 = s.eVar5 = s.getCk(k) : '';
		s.tl(this, "o" ,s.prop5);
		s.setCk('sc_onck' ,'');
	} else {
		return;
...
window.onunload = function() {
	sc_countRead('sc_onck');
}
s.getCk('sc_onck') ? s.eVar5 = s.prop5 = s.getCk('sc_onck') : '';

/* Subdomain JS selection */
function getScSubdom(){
  var sc_val=location.host;
  sc_val=sc_val.replace(/\.gnavi\.co\.jp/,"");
  return sc_val;
}
/************************** PLUGINS SECTION *************************/
...
+"v',cn);var pxh=vh+'/'+dh;s.c_w('s_pxh',pxh);");
